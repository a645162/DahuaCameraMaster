                ...
                for line in lines[1:]:  # 跳过标题行
                    if line.strip() and "," in line:
                        ...
                        if len(parts) >= 4:
                            ...
                            connection_id = (
                                parts[3].strip() if parts[3].strip() else "未知"
                            )
                                ...
                                adapters.append(
                                    {
                                        ...
                                        "mac": mac,
                                        "display_name": f"{connection_id} ({name[:30]}...)"
                                        if len(name) > 30
                                        else f"{connection_id} ({name})",
                                    }
                                )
    ...
    def set_static_ip(self) -> tuple[bool, str]:
        ...
        try:
            ...
            result_ip = subprocess.run(
                cmd_ip, shell=True, capture_output=True, text=True, encoding="gbk"
            )
            ...
            result_dns = subprocess.run(
                cmd_dns, shell=True, capture_output=True, text=True, encoding="gbk"
            )
...
class IPConfigWindow(QMainWindow, Ui_IPConfigWindow):
    ...
    def restore_dhcp(self):
        ...
        # 启动DHCP恢复线程
        self.config_thread = IPConfigThread(
            self.current_adapter["connection_id"], "", "", "", "", is_dhcp=True
        )
